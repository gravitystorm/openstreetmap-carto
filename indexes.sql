...
CREATE INDEX IF NOT EXISTS planet_osm_point_place_low
  ON planet_osm_point USING gist (way)
  WHERE place IN ('city', 'town') AND name IS NOT NULL;

CREATE INDEX IF NOT EXISTS planet_osm_point_place
  ON planet_osm_point USING gist (way)
  WHERE place IS NOT NULL AND name IS NOT NULL;

CREATE INDEX IF NOT EXISTS planet_osm_point_name
  ON planet_osm_point USING gist (way)
  WHERE name IS NOT NULL;

-- Most rows in the line table are highways, but there are some indexes useful
-- for low zooms
CREATE INDEX IF NOT EXISTS planet_osm_line_name
  ON planet_osm_line USING gist (way)
  WHERE name IS NOT NULL;

CREATE INDEX IF NOT EXISTS planet_osm_line_ferry
  ON planet_osm_line USING gist (way)
  WHERE route = 'ferry';

CREATE INDEX IF NOT EXISTS planet_osm_line_river
  ON planet_osm_line USING gist (way)
  WHERE waterway = 'river';

-- The polygon table is by far the largest, and generally the slowest
CREATE INDEX IF NOT EXISTS planet_osm_polygon_name
  ON planet_osm_polygon USING gist (way)
  WHERE name IS NOT NULL;

CREATE INDEX IF NOT EXISTS planet_osm_polygon_nobuilding
  ON planet_osm_polygon USING gist (way)
  WHERE building IS NULL;

CREATE INDEX IF NOT EXISTS planet_osm_polygon_military
  ON planet_osm_polygon USING gist (way)
...
CREATE INDEX IF NOT EXISTS planet_osm_roads_admin_low
...
CREATE INDEX IF NOT EXISTS planet_osm_roads_road_ref
  ON planet_osm_roads USING gist (way)
